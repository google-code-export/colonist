...
#region Edit AI
    ...
    {
        GUILayout.Label(new GUIContent("Edit Unit Basic Property------------------------------------------", "在此编辑单位的基础属性"));
                ...
                switch (AttackData.Type)
                {
                    case AIAttackType.Instant:
                        AttackData.HitTime = EditorGUILayout.FloatField(new GUIContent("Hit time:",
                            @"如果攻击类型 = Instant,从开始播放攻击动画算起,发送Apply Damage的延时;
如果攻击类型 = Projectile, 从开始播放攻击动画算起, 创建Projectile对象的延时."),
                            AttackData.HitTime);
                    ...
                    case AIAttackType.Projectile:
                        ...
                        AttackData.ProjectileInstantiateAnchor = (Transform)EditorGUILayout.ObjectField(new GUIContent("*Projectile Instantiate Anchor :", "创建射弹对象的Transform"), AttackData.ProjectileInstantiateAnchor, typeof(Transform));
                        break;
    ...
    public virtual void EditDeathData()
    {
        ...
        if (EnableEditDeathData)
        {
            ...
            for (int i = 0; i < AI.Unit.DeathData.Length; i++)
            {
                ...
                {
                    DeathData.ReplaceAfterAnimationFinish = EditorGUILayout.Toggle(new GUIContent("Create replacement following animation", "是否在动画结束之后创建替代物?"), DeathData.ReplaceAfterAnimationFinish);
    ...
    public virtual void EditBaseAIProperty()
    {
        ...
        AI.AlterBehaviorInterval = EditorGUILayout.FloatField(new GUIContent("Behavior alternation time",
            "AI行为轮替周期,不推荐修改这个值."), AI.AlterBehaviorInterval);
    }
    ...
    public virtual void EditAIBehaviorData(AIBehavior behavior)
    {
        ...
        switch (behavior.Type)
        {
            ...
            case AIBehaviorType.HoldPosition:
                ...
                {
                    behavior.HoldRadius = EditorGUILayout.FloatField(new GUIContent("Hold Position:", "坚守阵地的范围"), behavior.HoldRadius);
                }
                break;
        }
    }
